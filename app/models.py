...
class AssetClass(Base):
    """Admin-editable list of asset classes (R&D Shared / AirJet / ...)."""
...
class UserDepartment(Base):
    ...
    Easy Auth email so it's remembered everywhere they log in."""
...
class Asset(Base):
    ...
    po_number = Column(String, nullable=True)     # purchase-order number for this asset
...
class Reminder(Base):
    ...
    # Tagging for the Personal / Department / All tabs:
    #   scope="personal"   -> shows only to the creator (by email)
    #   scope="department" -> shows to everyone in department_id
    scope = Column(String(20), nullable=False, default="department")
    department_id = Column(Integer, ForeignKey("departments.id"), nullable=True)
