...
def list_locations(db: Session = Depends(get_db), user=Depends(current_user)):
    ...
    children nested. Also returns a flat list for convenience."""
...
def create_location(body: dict, db: Session = Depends(get_db), user=Depends(require_admin)):
    ...
    if parent_id:
        ...
        # keep it two levels: a child can't itself be a parent's child's parent
...
def update_class(cls_id: int, body: dict, db: Session = Depends(get_db), user=Depends(require_admin)):
    ...
    if not name:
        raise HTTPException(400, "Name is required")
    c.name = name; db.commit(); db.refresh(c)
    return {"id": c.id, "name": c.name}
...
def get_my_department(db: Session = Depends(get_db), user=Depends(current_user)):
    """The signed-in user's chosen department, remembered by their email."""
    email = user.email
    if not email:
...
def activity_feed(limit: int = 50, before: str = "", db: Session = Depends(get_db),
    ...
    note_q = (db.query(models.CheckIn)
              .filter(models.CheckIn.note.isnot(None), models.CheckIn.note != "",
                      # Exclude the auto-generated "Inventory check" note that a
                      # barcode/QR scan attaches - it would otherwise flood the
                      # feed. Genuine notes a person types still show.
...
IMPORT_FIELDS = ["name", "asset_code", "asset_class", "department", "owner",
                 "vendor", "location", "cost", "serial_number", "po_number",
...
def _parse_import(raw, sheet, header_row, mapping, dept_lookup, existing_codes=None):
    ...
    dup_skipped = 0
    seen_codes = set()          # also dedupe within the same sheet
    for i, row in enumerate(data):
        ...
        # Dedup by Asset Code: if this row has a code that already exists (in the
        # database or earlier in this same sheet), skip it so re-importing one
        # master spreadsheet only adds genuinely new assets. Rows with NO code
        # are always imported (per request), so a codeless row can repeat.
        code = _clean_str(cell("asset_code"))
        if code:
            key = code.upper()
            if key in existing_codes or key in seen_codes:
                dup_skipped += 1
                continue
            seen_codes.add(key)
        ...
        if code:
            f["asset_code"] = code
        for plain in ("owner", "vendor", "location", "serial_number", "po_number", "notes"):
    ...
    return assets, warnings, skipped, len(data), dup_skipped
...
def _make_asset(db, f, user_email):
    ...
    # Preserve the spreadsheet's Asset Code when present so re-imports match it
    # for dedup; otherwise the model generates its own random code.
    if f.get("asset_code"):
        kwargs["code"] = f["asset_code"].upper()
    a = models.Asset(**kwargs)
    db.add(a); db.flush()
...
async def import_preview(file: UploadFile = File(...), header_row: int = Form(1),
    ...
    existing_codes = {c[0].upper() for c in db.query(models.Asset.code).all() if c[0]}
    assets, warnings, skipped, total, dup_skipped = _parse_import(
        raw, sheet, header_row, m, dept_lookup, existing_codes)
    return {"total_rows": total, "will_import": len(assets), "skipped_blank": skipped,
            "skipped_duplicate": dup_skipped,
...
async def import_commit(file: UploadFile = File(...), header_row: int = Form(1),
    ...
    return {"created": len(assets), "skipped_blank": skipped,
            "skipped_duplicate": dup_skipped, "warning_count": len(warnings)}
...
def _reminders_due_count(a: models.Asset) -> int:
    ...
    return sum(1 for r in a.reminders_list
               if r.active and _as_date(r.next_due) and _as_date(r.next_due) <= today)


def _as_date(value):
    """Coerce a stored temporal value to a plain date.

    SQLite hands back datetime.date for DATE columns, but SQL Server (via pyodbc)
    can return a datetime instead. Subtracting a date from a datetime throws
    TypeError -> 500, which is what broke reminder "Mark done"/list on Azure. This
    normalises date / datetime / ISO-string to a date so the math always works.
    """
    if value is None:
        return None
    ...
    if isinstance(value, date_type):
        return value
    try:
        return _dt.fromisoformat(str(value)[:19]).date()
    except Exception:
        return None
...
def _reminder_brief(r: models.Reminder):
    ...
    return {"id": r.id, "asset_id": r.asset_id, "label": r.label, "kind": r.kind,
            ...
            "note": r.note, "last_done_at": ld.isoformat() if ld else None,
            "scope": r.scope, "department_id": r.department_id,
...
def _create_one_reminder(db, asset_id, body, user_email):
    ...
    scope = body.get("scope") if body.get("scope") in ("personal", "department") else "department"
    # A department reminder needs a department; default to the creator's own.
    dept_id = body.get("department_id")
    if scope == "department" and not dept_id and user_email:
        ud = db.get(models.UserDepartment, user_email)
        dept_id = ud.department_id if ud else None
    r = models.Reminder(asset_id=asset_id, label=label, kind=kind,
                        note=(body.get("note") or "").strip() or None,
                        created_by=user_email, scope=scope,
...
def complete_reminder(reminder_id: int, body: dict, db: Session = Depends(get_db),
    ...
    if r.kind == "interval" and r.interval_days:
        base = _as_date(r.next_due) or done_on
...
@router.get("/reminders/feed")
def reminders_feed(scope: str = "all", db: Session = Depends(get_db), user=Depends(current_user)):
    """Reminder lists for the tabbed view.
      scope=personal   -> reminders this user created as personal
      scope=department -> department reminders for this user's department
      scope=all        -> every department reminder across the company
    Each item includes its asset and department name so the UI can show context.
    """
    email = user.email
    q = db.query(models.Reminder).filter(models.Reminder.active == True,
                                         models.Reminder.next_due.isnot(None))
    if scope == "personal":
        q = q.filter(models.Reminder.scope == "personal",
                     models.Reminder.created_by == email)
    elif scope == "department":
        ud = db.get(models.UserDepartment, email) if email else None
        my_dept = ud.department_id if ud else None
        if not my_dept:
            return {"scope": scope, "reminders": [], "no_department": True}
        q = q.filter(models.Reminder.scope == "department",
                     models.Reminder.department_id == my_dept)
    else:  # all
        q = q.filter(models.Reminder.scope == "department")

    dept_names = {d.id: d.name for d in db.query(models.Department).all()}
    out = []
    for r in q.order_by(models.Reminder.next_due).all():
        ...
        item["department"] = dept_names.get(r.department_id)
        out.append(item)
    return {"scope": scope, "reminders": out}
...
def export_assets_xlsx(q: Optional[str] = None, db: Session = Depends(get_db),
    ...
    headers = ["Name", "Asset Code", "Department", "Owner", "Vendor",
              "Location", "Serial Number", "PO #", "Cost", "Date Purchased",
    ...
    for i, w in enumerate([26, 14, 16, 16, 16, 18, 16, 14, 10, 14, 14, 22, 20, 14, 30], 1):
...
def update_asset(asset_id: int, body: dict, db: Session = Depends(get_db), user=Depends(require_support)):
    ...
    for f in ("name", "location", "owner", "vendor",
             "serial_number", "po_number", "asset_class", "notes", "active"):
        if f in body:
            setattr(a, f, body[f])
    if "cost" in body:
