...
# The five test columns already present in the source workbook. Minis tracked
# RnD FTT/HDT/StepC; Floaters tracked Hot Poling/RnD STT/RnD FTT. Seeded as one
# combined list since criteria are shared across campaigns and admins can
# hide whichever don't apply.
DEFAULT_TEST_CRITERIA = [
    ("hot_poling", "Hot Poling"),
    ("rnd_stt",    "RnD STT"),
    ("rnd_ftt",    "RnD FTT"),
    ("rnd_hdt",    "RnD HDT"),
    ("rnd_stepc",  "RnD StepC"),
...
def seed_test_criteria(db: Session):
    """Intentionally seeds nothing.

    Test columns differ by team and by campaign, so shipping "Hot Poling /
    RnD STT / ..." as defaults just meant everyone had to delete them first.
    The grid starts with no test columns and people add their own via
    "+ Field".
    """
    return
