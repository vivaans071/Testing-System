...
from .seed import (seed_departments, seed_locations, seed_classes,
                   seed_statuses, seed_test_criteria, normalize_flat_locations)
...
def _ensure_schema():
    ...
    # ── Columns added to reminders for email notifications ──
    if "reminders" in tables:
        cols = [c["name"] for c in insp.get_columns("reminders")]
        with engine.begin() as conn:
            add = "ALTER TABLE reminders ADD " if is_mssql else "ALTER TABLE reminders ADD COLUMN "
            TXT = "NVARCHAR(MAX)" if is_mssql else "TEXT"
            for col, typ in [("email_notify", BOOL), ("notify_emails", TXT),
                             ("notified_7d", BOOL), ("notified_1d", BOOL),
                             ("notified_due", BOOL)]:
                if col not in cols:
                    conn.execute(text(f"{add}{col} {typ}"))

    # ── Columns added to the testing tables ──
    if "test_sample_minis" in tables:
        cols = [c["name"] for c in insp.get_columns("test_sample_minis")]
        with engine.begin() as conn:
            add = "ALTER TABLE test_sample_minis ADD " if is_mssql else "ALTER TABLE test_sample_minis ADD COLUMN "
            if "location" not in cols:
                conn.execute(text(f"{add}location {'NVARCHAR(255)' if is_mssql else 'VARCHAR(255)'}"))
            if "tests" not in cols:
                conn.execute(text(f"{add}tests {'NVARCHAR(MAX)' if is_mssql else 'TEXT'}"))

    if "test_sample_lots" in tables:
        cols = [c["name"] for c in insp.get_columns("test_sample_lots")]
        with engine.begin() as conn:
            add = "ALTER TABLE test_sample_lots ADD " if is_mssql else "ALTER TABLE test_sample_lots ADD COLUMN "
            STR255 = "NVARCHAR(255)" if is_mssql else "VARCHAR(255)"
            DT = "DATETIME2" if is_mssql else "DATETIME"
            for col, typ in [("checked_out", BOOL), ("held_by", STR255), ("checked_out_at", DT)]:
                if col not in cols:
                    conn.execute(text(f"{add}{col} {typ}"))

    # ── Columns added to asset_statuses for color + popup note ──
    if "asset_statuses" in tables:
        cols = [c["name"] for c in insp.get_columns("asset_statuses")]
        with engine.begin() as conn:
            add = "ALTER TABLE asset_statuses ADD " if is_mssql else "ALTER TABLE asset_statuses ADD COLUMN "
            TXT = "NVARCHAR(MAX)" if is_mssql else "TEXT"
            if "color" not in cols:
                conn.execute(text(f"{add}color {'NVARCHAR(20)' if is_mssql else 'VARCHAR(20)'}"))
            if "popup_note" not in cols:
                conn.execute(text(f"{add}popup_note {TXT}"))
