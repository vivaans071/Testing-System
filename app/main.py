...
def _ensure_schema():
    ...
    if "assets" in tables:
        ...
        with engine.begin() as conn:
            ...
            if "po_number" not in cols:
                conn.execute(text(f"ALTER TABLE assets ADD po_number {STR}"
                                  if is_mssql else "ALTER TABLE assets ADD COLUMN po_number VARCHAR"))
    ...
    if "locations" in tables:
        ...
        if "parent_id" not in cols:
            with engine.begin() as conn:
                add = "ALTER TABLE locations ADD " if is_mssql else "ALTER TABLE locations ADD COLUMN "
                conn.execute(text(f"{add}parent_id {INT}"))
    if "reminders" in tables:
        ...
        with engine.begin() as conn:
            ...
            if "scope" not in cols:
                conn.execute(text(f"{add}scope {'NVARCHAR(20)' if is_mssql else 'VARCHAR(20)'} "
                                  f"{'DEFAULT ' if is_mssql else 'DEFAULT '}'department'"))
            if "department_id" not in cols:
                conn.execute(text(f"{add}department_id {INT}"))
