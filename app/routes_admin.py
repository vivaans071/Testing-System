...
def delete_department(dept_id: int, db: Session = Depends(get_db), user=Depends(require_admin)):
    ...
    if not d:
        raise HTTPException(404, "Department not found")
    # Three tables point at a department, not just Asset.department_id. Missing
    # any of them leaves an orphan reference and the delete fails on the foreign
    # key (SQL Server enforces this; SQLite silently doesn't).
    db.query(models.Asset).filter(models.Asset.department_id == dept_id) \
        .update({"department_id": None})
    db.query(models.Asset).filter(models.Asset.holder_department_id == dept_id) \
        .update({"holder_department_id": None})
    db.query(models.CustodyEvent).filter(models.CustodyEvent.department_id == dept_id) \
        .update({"department_id": None})
...
def list_locations(db: Session = Depends(get_db), user=Depends(current_user)):
    """Returns locations as a two-level tree: each top-level location with its
    children nested. Also returns a flat list for populating dropdowns."""
    rows = db.query(models.Location).order_by(models.Location.name).all()
    by_id = {l.id: {"id": l.id, "name": l.name, "parent_id": l.parent_id, "children": []}
             for l in rows}
    tree = []
    for l in rows:
        node = by_id[l.id]
        if l.parent_id and l.parent_id in by_id:
            by_id[l.parent_id]["children"].append(node)
        else:
            tree.append(node)
    flat = [{"id": l.id, "name": l.name, "parent_id": l.parent_id} for l in rows]
    return {"tree": tree, "flat": flat}


@router.post("/locations")
def create_location(body: dict, db: Session = Depends(get_db), user=Depends(require_admin)):
    name = (body.get("name") or "").strip()
    if not name:
        raise HTTPException(400, "Name is required")
    parent_id = body.get("parent_id")
    # Unique within the same parent only - "104" may exist under both
    # Acoustic and Metrology without clashing.
    sibling = (db.query(models.Location)
               .filter(models.Location.name == name,
                       models.Location.parent_id == (int(parent_id) if parent_id else None))
               .first())
    if sibling:
        raise HTTPException(400, "That location already exists here")
    if parent_id:
        parent = db.get(models.Location, int(parent_id))
        if not parent:
            raise HTTPException(400, "Parent location not found")
        if parent.parent_id is not None:
            raise HTTPException(400, "Locations only go two levels deep.")
    l = models.Location(name=name, parent_id=int(parent_id) if parent_id else None)
    db.add(l); db.commit(); db.refresh(l)
    return {"id": l.id, "name": l.name, "parent_id": l.parent_id}


@router.put("/locations/{loc_id}")
def update_location(loc_id: int, body: dict, db: Session = Depends(get_db), user=Depends(require_admin)):
    l = db.get(models.Location, loc_id)
    if not l:
    ...
    if "name" in body:
        ...
        if not name:
            raise HTTPException(400, "Name is required")
        clash = (db.query(models.Location)
                 .filter(models.Location.name == name,
                         models.Location.parent_id == l.parent_id,
                         models.Location.id != loc_id).first())
        if clash:
            raise HTTPException(400, "Another location here already has that name")
        old_name = l.name
        l.name = name
        # Assets store the composite label ("Metrology - PZT"), so rename what
        # actually appears on the asset, not the bare row name. Renaming a
        # parent has to rewrite every one of its children's labels too.
        def _relabel(before, after):
            if before != after:
                db.query(models.Asset).filter(models.Asset.location == before) \
                    .update({"location": after})

        if l.parent_id:
            parent = db.get(models.Location, l.parent_id)
            head = parent.name if parent else ""
            _relabel(f"{head} - {old_name}", f"{head} - {name}")
        else:
            _relabel(old_name, name)
            for child in db.query(models.Location).filter(
                    models.Location.parent_id == loc_id).all():
                _relabel(f"{old_name} - {child.name}", f"{name} - {child.name}")
...
def delete_location(loc_id: int, db: Session = Depends(get_db), user=Depends(require_admin)):
    ...
    if not l:
        raise HTTPException(404, "Location not found")
    # deleting a parent also removes its children (they can't stand alone)
    db.query(models.Location).filter(models.Location.parent_id == loc_id).delete()
...
# --------------------------------------------------------------- classes
@router.get("/classes")
def list_classes(db: Session = Depends(get_db), user=Depends(current_user)):
    rows = db.query(models.AssetClass).order_by(models.AssetClass.name).all()
    return [{"id": c.id, "name": c.name} for c in rows]


@router.post("/classes")
def create_class(body: dict, db: Session = Depends(get_db), user=Depends(require_admin)):
    name = (body.get("name") or "").strip()
    if not name:
        raise HTTPException(400, "Name is required")
    if db.query(models.AssetClass).filter(models.AssetClass.name == name).first():
        raise HTTPException(400, "That class already exists")
    c = models.AssetClass(name=name); db.add(c); db.commit(); db.refresh(c)
    return {"id": c.id, "name": c.name}


@router.put("/classes/{cls_id}")
def update_class(cls_id: int, body: dict, db: Session = Depends(get_db), user=Depends(require_admin)):
    c = db.get(models.AssetClass, cls_id)
    if not c:
        raise HTTPException(404, "Class not found")
    name = (body.get("name") or "").strip()
    if not name:
        raise HTTPException(400, "Name is required")
    clash = (db.query(models.AssetClass)
             .filter(models.AssetClass.name == name, models.AssetClass.id != cls_id).first())
    if clash:
        raise HTTPException(400, "Another class already has that name")
    old_name = c.name
    c.name = name
    db.query(models.Asset).filter(models.Asset.asset_class == old_name) \
        .update({"asset_class": name})
    db.commit(); db.refresh(c)
    return {"id": c.id, "name": c.name}


@router.delete("/classes/{cls_id}")
def delete_class(cls_id: int, db: Session = Depends(get_db), user=Depends(require_admin)):
    c = db.get(models.AssetClass, cls_id)
    if not c:
        raise HTTPException(404, "Class not found")
...
def get_my_department(db: Session = Depends(get_db), user=Depends(current_user)):
    """The signed-in user's own department. needs_setup is True only when no
    row exists at all, so someone who deliberately picked "none" isn't asked
    again every time they load the page."""
    email = user.email
    if not email:
        return {"email": None, "department_id": None, "needs_setup": False}
    ud = db.get(models.UserDepartment, email)
    return {"email": email,
            "department_id": ud.department_id if ud else None,
            "needs_setup": ud is None}


@router.put("/my-department")
def set_my_department(body: dict, db: Session = Depends(get_db), user=Depends(current_user)):
    email = user.email
    if not email:
        raise HTTPException(400, "No signed-in user (is Easy Auth enabled?).")
    dept_id = body.get("department_id")
    if dept_id:
        if not db.get(models.Department, int(dept_id)):
            raise HTTPException(400, "Unknown department.")
    ud = db.get(models.UserDepartment, email)
    if not ud:
        ud = models.UserDepartment(email=email)
        db.add(ud)
    ud.department_id = int(dept_id) if dept_id else None
    db.commit()
    return {"email": email, "department_id": ud.department_id}


def _status_names(db: Session):
    """The live status list, falling back to the hardcoded defaults if the
...
@router.get("/statuses")
def list_statuses(db: Session = Depends(get_db), user=Depends(current_user)):
    rows = (db.query(models.AssetStatus)
            .order_by(models.AssetStatus.sort_order, models.AssetStatus.id).all())
    if not rows:
        return [{"id": None, "name": n, "color": None, "popup_note": None}
                for n in models.STATUSES]
    return [{"id": r.id, "name": r.name, "color": r.color, "popup_note": r.popup_note}
            for r in rows]


@router.post("/statuses")
def create_status(body: dict, db: Session = Depends(get_db), user=Depends(require_admin)):
    name = (body.get("name") or "").strip()
    if not name:
        raise HTTPException(400, "Name is required")
    if db.query(models.AssetStatus).filter(models.AssetStatus.name == name).first():
        raise HTTPException(400, "That status already exists")
    top = db.query(func.max(models.AssetStatus.sort_order)).scalar() or 0
    r = models.AssetStatus(name=name, sort_order=top + 1,
                           color=(body.get("color") or "").strip() or None,
                           popup_note=(body.get("popup_note") or "").strip() or None)
    db.add(r); db.commit(); db.refresh(r)
    return {"id": r.id, "name": r.name, "color": r.color, "popup_note": r.popup_note}


@router.put("/statuses/{status_id}")
def update_status_name(status_id: int, body: dict, db: Session = Depends(get_db),
                       user=Depends(require_admin)):
    r = db.get(models.AssetStatus, status_id)
    if not r:
        raise HTTPException(404, "Status not found")
    # Name is only validated when it's actually being changed.
    if "name" in body:
        name = (body.get("name") or "").strip()
        if not name:
            raise HTTPException(400, "Name is required")
        clash = (db.query(models.AssetStatus)
                 .filter(models.AssetStatus.name == name, models.AssetStatus.id != status_id).first())
        if clash:
            raise HTTPException(400, "Another status already has that name")
        old = r.name
        r.name = name
        db.query(models.Asset).filter(models.Asset.status == old).update({"status": name})
    if "color" in body:
        r.color = (body.get("color") or "").strip() or None
    if "popup_note" in body:
        r.popup_note = (body.get("popup_note") or "").strip() or None
    db.commit(); db.refresh(r)
    return {"id": r.id, "name": r.name, "color": r.color, "popup_note": r.popup_note}


@router.delete("/statuses/{status_id}")
def delete_status(status_id: int, db: Session = Depends(get_db), user=Depends(require_admin)):
    r = db.get(models.AssetStatus, status_id)
    if not r:
        raise HTTPException(404, "Status not found")
    in_use = db.query(func.count(models.Asset.id)).filter(models.Asset.status == r.name).scalar()
    if in_use:
        raise HTTPException(400,
            f"{in_use} asset{'s' if in_use != 1 else ''} still use '{r.name}'. "
            "Move them to another status first.")
    if db.query(func.count(models.AssetStatus.id)).scalar() <= 1:
...
# --------------------------------------------------------------- purchase orders
@router.get("/assets/{asset_id}/pos")
def list_pos(asset_id: int, db: Session = Depends(get_db), user=Depends(current_user)):
    rows = (db.query(models.PurchaseOrder)
            .filter(models.PurchaseOrder.asset_id == asset_id)
            .order_by(models.PurchaseOrder.created_at.desc()).all())
    return [{"id": p.id, "po_number": p.po_number, "cost": p.cost, "note": p.note,
             "created_at": p.created_at.isoformat(timespec="seconds") if p.created_at else None,
             "created_by": p.created_by} for p in rows]


@router.post("/assets/{asset_id}/pos")
def add_po(asset_id: int, body: dict, db: Session = Depends(get_db), user=Depends(current_user)):
    ...
    p = models.PurchaseOrder(asset_id=asset_id, po_number=po_number, cost=cost,
                             note=(body.get("note") or "").strip() or None,
                             created_by=user.email)
    db.add(p); db.commit(); db.refresh(p)
    return {"id": p.id, "po_number": p.po_number, "cost": p.cost, "note": p.note,
            "created_at": p.created_at.isoformat(timespec="seconds"), "created_by": p.created_by}


@router.delete("/pos/{po_id}")
def delete_po(po_id: int, db: Session = Depends(get_db), user=Depends(current_user)):
    p = db.get(models.PurchaseOrder, po_id)
    if not p:
...
def activity_feed(limit: int = 50, before: str = "", db: Session = Depends(get_db),
    ...
    note_q = (db.query(models.CheckIn)
              .filter(models.CheckIn.note.isnot(None), models.CheckIn.note != "",
                      # see activity_unread: auto-stamped barcode sweeps stay
                      # in the asset's own history but out of the feed
                      ~((models.CheckIn.source == "barcode_scan")
                        & (models.CheckIn.note == "Inventory check"))))
    ...
    # A scanned check-in that changes status and carries a note writes both a
    # CheckIn note row and a StatusChange row with the same text and timestamp.
    # Show only the status row - the note travels with it - so the feed doesn't
    # list the same action twice.
    status_events = [e for e in events if e["type"] == "status"]
    def _dupe_of_status(ev):
        if ev["type"] != "note" or not ev.get("note"):
            return False
        n_ts = _dt.fromisoformat(ev["ts"])
        for se in status_events:
            if se["asset_id"] != ev["asset_id"]:
                continue
            if (se.get("note") or "").strip() != (ev["note"] or "").strip():
                continue
            if abs((_dt.fromisoformat(se["ts"]) - n_ts).total_seconds()) <= 120:
                return True
        return False
    events = [e for e in events if not _dupe_of_status(e)]
...
def activity_mark_read(db: Session = Depends(get_db), user=Depends(current_user)):
    """Clear this user's notifications. Stamps 'now' rather than the newest
    event's timestamp, so everything that exists at this moment is covered
    regardless of clock precision - which is what makes clicking the bell
    actually clear the badge instead of leaving a few stragglers."""
    email = user.email
    if not email:
        return {"ok": True, "seen_at": None, "stored": False}
    row = db.get(models.UserActivitySeen, email)
    if not row:
        row = models.UserActivitySeen(email=email)
        db.add(row)
    row.seen_at = models.utcnow()
    db.commit()
    return {"ok": True, "seen_at": row.seen_at.isoformat(), "stored": True}
...
def activity_unread(since: str = "", db: Session = Depends(get_db),
    ...
    Prefers this user's server-side marker; `since` is only a fallback for
    sessions with no signed-in email (Easy Auth off, local testing)."""
    # Barcode inventory sweeps auto-stamp "Inventory check" on every scan.
    # They belong in each asset's own history but would otherwise flood the
    # notification bell, so they're excluded from the feed and its count.
    ...
    dt = None
    row = db.get(models.UserActivitySeen, user.email) if user.email else None
    ...
    # Full precision on purpose - a second-truncated marker would leave every
    # event inside that same second permanently newer than it.
    return {"count": count,
            "latest": latest.isoformat() if latest else None,
            "server_marker": bool(row and row.seen_at)}


# --------------------------------------------------------------- excel import
IMPORT_FIELDS = ["name", "asset_code", "asset_class", "department", "owner",
...
def _dedupe_name(name, taken_lower):
    """If `name` (case-insensitive) is already taken, append a numbered suffix
    until it isn't. Numbering counts the original as #1, so the suffix matches
    how many of that name exist: the second "Oven" becomes "Oven (2)", the
    fifth "Oven (5)". Starting at (1) would have made five ovens read
    Oven .. Oven (4) and look like one fewer than there really is.

    Mutates taken_lower with whatever it returns."""
...
def _parse_import(raw, sheet, header_row, mapping, dept_lookup, existing_codes, existing_names):
    ...
    seen_codes = set(existing_codes)     # grows as the batch is walked, so two
    taken_names = set(existing_names)    # rows in the same file also de-dup

    assets, warnings, skipped_blank, skipped_dupe_code = [], [], 0, 0
    for i, row in enumerate(data):
        ...
        # Asset Code is the de-dup key. If it matches an asset that already
        # exists (or one already queued earlier in this same file), skip the
        # row entirely rather than creating a duplicate - this is what keeps
        # a re-import of a previously-exported sheet from ballooning the
        # catalog with copies every time it's run.
        raw_code = _clean_str(cell("asset_code"))
        norm_code = raw_code.upper() if raw_code else None
        ...
        # Names aren't required to be unique, but two active assets with the
        # identical name are confusing - disambiguate instead of erroring.
        deduped_name = _dedupe_name(f["name"], taken_names)
        if deduped_name != f["name"]:
            msgs.append(f"name '{f['name']}' already exists — imported as '{deduped_name}'")
        f["name"] = deduped_name

        if norm_code:
            f["asset_code"] = norm_code
    ...
    return assets, warnings, skipped_blank, skipped_dupe_code, len(data)
...
def _make_asset(db, f, user_email):
    ...
    if f.get("asset_code"):
        # Only set when the sheet provided one and it didn't already exist
        # (see _parse_import) - otherwise leave it unset so the model's
        # default (a fresh random code) applies.
        a.code = f["asset_code"]
    db.add(a); db.flush()
...
async def import_preview(file: UploadFile = File(...), header_row: int = Form(1),
    ...
    existing_codes = {c.upper() for (c,) in db.query(models.Asset.code).all() if c}
    existing_names = {n.lower() for (n,) in db.query(models.Asset.name)
                      .filter(models.Asset.active == True).all() if n}
    assets, warnings, skipped, skipped_dupe_code, total = _parse_import(
        raw, sheet, header_row, m, dept_lookup, existing_codes, existing_names)
    return {"total_rows": total, "will_import": len(assets), "skipped_blank": skipped,
            "skipped_duplicate_code": skipped_dupe_code,
...
async def import_commit(file: UploadFile = File(...), header_row: int = Form(1),
    ...
    return {"created": len(assets), "skipped_blank": skipped,
            "skipped_duplicate_code": skipped_dupe_code, "warning_count": len(warnings)}
...
def analytics(exclude_missing: bool = False,
    ...
    return {
        ...
        "by_status": [{"label": s, "count": by_status.get(s, 0)} for s in _status_names(db)
                      if by_status.get(s)],
...
def bulk_status(body: dict, db: Session = Depends(get_db), user=Depends(current_user)):
    ...
    if status not in _status_names(db):
...
@router.post("/assets/bulk-edit")
def bulk_edit(body: dict, db: Session = Depends(get_db), user=Depends(require_support)):
    """Set the same field values on many assets at once.

    Only the keys actually present in body["fields"] are touched, so a caller
    can change just the owner without blanking anything else. An explicit null
    or "" clears that field - that's why presence matters rather than
    truthiness. Status is deliberately not handled here: it has its own
    endpoint (/assets/bulk-status) that logs each transition and fires the
    ...
    allowed = {"owner", "vendor", "location", "asset_class", "department_id", "notes"}
    ...
    for key, val in fields.items():
        if key == "department_id":
            if val in (None, "", 0):
                ...
                dept = db.get(models.Department, int(val))
                if not dept:
                    raise HTTPException(400, "Unknown department.")
                clean["department_id"] = dept.id
        else:
    ...
    for a in rows:
        for key, val in clean.items():
            setattr(a, key, val)
    db.commit()
    return {"updated": len(rows), "fields": sorted(clean)}
...
def export_assets_xlsx(q: Optional[str] = None, department_ids: Optional[str] = None,
    ...
    # Deactivated assets are deliberately excluded - the sheet is a working
    # inventory list, and re-importing it shouldn't resurrect retired kit.
    assets = _filter_assets(
        db.query(models.Asset).filter(models.Asset.active == True)
        .order_by(models.Asset.name).all(), q)

    # Optional department filter. "none" is an accepted member of the list and
    # means "assets with no department", so that group can be exported too.
    if department_ids is not None:
        wanted, want_none = set(), False
        for part in department_ids.split(","):
            part = part.strip()
            if not part:
                continue
            if part.lower() == "none":
                ...
                try:
                    ...
                    raise HTTPException(400, f"Bad department id '{part}'.")
        if not wanted and not want_none:
            raise HTTPException(400, "No departments selected.")
        assets = [a for a in assets
                  if (a.department_id in wanted) or (want_none and not a.department_id)]

    wb = Workbook()
    ws = wb.active
    ws.title = "Assets"
    # Department is column A so the group heading sits at the far left and the
    # Name column can be left EMPTY on heading rows. That's what makes this
    # sheet safe to re-import: _parse_import treats a blank Name as a
    # banner/spacer row and skips it, so the headings never come back as assets.
    headers = ["Department", "Name", "Asset Code", "Owner", "Vendor",
    ...
    # Group by department, each group introduced by a bold banner row. Assets
    # with no department sort last under "(No department)".
    groups = {}
    for a in assets:
        key = a.department.name if a.department else None
        groups.setdefault(key, []).append(a)
    ordered = sorted([k for k in groups if k is not None], key=lambda s: s.lower())
    if None in groups:
        ordered.append(None)

    for dept_name in ordered:
        rows = groups[dept_name]
        label = dept_name or "(No department)"
        ws.append([f"{label}  ({len(rows)})"])   # column A only; Name stays blank
        banner = ws.cell(row=ws.max_row, column=1)
        banner.font = Font(bold=True, color="0B1F3A", size=12)
        for col in range(1, len(headers) + 1):
            ws.cell(row=ws.max_row, column=col).fill = PatternFill("solid", start_color="DCE6F1")
        for a in rows:
            last = a.checkins[0] if a.checkins else None
            ws.append([
                a.department.name if a.department else "", a.name, a.code,
            ...
            # indent the asset name so it reads as nested under the banner
            ws.cell(row=ws.max_row, column=2).alignment = Alignment(indent=2)

    for i, w in enumerate([18, 26, 14, 16, 16, 18, 16, 10, 14, 14, 22, 20, 14, 30], 1):
...
@router.get("/assets/export-print.xlsx")
def export_print_xlsx(ids: str, db: Session = Depends(get_db), user=Depends(current_user)):
    """Slim export for Brady Workstation label printing: just the barcode
    value (asset code) and QR value (check-in URL) for the selected assets.
    Must stay registered before /assets/{asset_id} - see export.xlsx above."""
    try:
        id_list = [int(x) for x in ids.split(",") if x.strip()]
    except ValueError:
        raise HTTPException(400, "ids must be a comma-separated list of asset ids")
    if not id_list:
        raise HTTPException(400, "No assets selected.")
    assets = (db.query(models.Asset).filter(models.Asset.id.in_(id_list))
             .order_by(models.Asset.name).all())
    if not assets:
        raise HTTPException(404, "None of the selected assets were found.")

    wb = Workbook()
    ws = wb.active
    ws.title = "Print"
    headers = ["Name", "Asset Code", "Check-In URL"]
    ws.append(headers)
    for c in ws[1]:
        c.font = Font(bold=True, color="FFFFFF")
        c.fill = PatternFill("solid", start_color="0B1F3A")
    for a in assets:
        ws.append([a.name, a.code, checkin_url(base_url(db), a.code)])
    for i, w in enumerate([32, 14, 42], 1):
        ws.column_dimensions[chr(64 + i)].width = w
    ws.freeze_panes = "A2"
    ...
    return StreamingResponse(
        buf, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": 'attachment; filename="export_to_print.xlsx"'})
...
def get_asset(asset_id: int, db: Session = Depends(get_db), user=Depends(current_user)):
    ...
    # A scan that changes the status AND carries a note used to produce two
    # rows: the real transition, plus a note-only row where old == new. Fold
    # the note-only row into the transition that happened alongside it so the
    # log shows one event with the note in its own column.
    def _is_note_only(h):
        return (h["type"] == "status_change" and h["old_status"]
                and h["old_status"] == h["new_status"])

    real_changes = [h for h in history
                    if h["type"] == "status_change" and not _is_note_only(h)]
    merged_ids = set()
    for note_row in [h for h in history if _is_note_only(h)]:
        # find a genuine transition within a short window of the note
        n_ts = _dt.fromisoformat(note_row["ts"])
        best, best_gap = None, None
        for change in real_changes:
            gap = abs((_dt.fromisoformat(change["ts"]) - n_ts).total_seconds())
            if gap <= 120 and (best_gap is None or gap < best_gap):
                best, best_gap = change, gap
        if best is not None:
            if note_row.get("note") and not best.get("note"):
                best["note"] = note_row["note"]
            merged_ids.add(id(note_row))
    history = [h for h in history if id(h) not in merged_ids]
    status_history = [h for h in status_history
                      if not (_is_note_only(h) and any(
                          abs((_dt.fromisoformat(h["ts"]) - _dt.fromisoformat(c["ts"])).total_seconds()) <= 120
