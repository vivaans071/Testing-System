...
def create_location(body: dict, db: Session = Depends(get_db), user=Depends(require_admin)):
    ...
    if db.query(models.Location).filter(models.Location.name == name).first():
        raise HTTPException(400, "That location already exists")
...
def update_location(loc_id: int, body: dict, db: Session = Depends(get_db), user=Depends(require_admin)):
    ...
    if "name" in body:
        ...
        clash = (db.query(models.Location)
                 .filter(models.Location.name == name, models.Location.id != loc_id).first())
        if clash:
            raise HTTPException(400, "Another location already has that name")
        old_name = l.name
        l.name = name
        # Asset.location stores the name as text, so rename in place rather
        # than orphaning every asset that pointed at the old label.
        db.query(models.Asset).filter(models.Asset.location == old_name) \
...
def activity_unread(since: str = "", db: Session = Depends(get_db),
    ...
    # Full precision on purpose. This value is what the UI stores as its
    # "seen up to here" marker, and the count below is a strict `ts > since`.
    # Truncating to seconds would leave every event inside that same second
    # permanently newer than the marker, so the badge could never clear.
    return {"count": count,
