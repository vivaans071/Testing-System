...
def _dedupe_name(name, taken_lower):
    """If `name` (case-insensitive) is already taken, append " (1)", " (2)"...
    until it isn't. Mutates taken_lower with whatever it returns."""
...
def export_assets_xlsx(q: Optional[str] = None, department_ids: Optional[str] = None,
    ...
    for dept_name in ordered:
        ...
        for a in rows:
            ...
            # indent the asset name one level so it reads as nested under the banner
            ws.cell(row=ws.max_row, column=1).alignment = Alignment(indent=2)
